...
export default async function handler(
  ...
  try {
    ...
    // All individual sales (used for chart & monthly)
    const allSales = await Sale.find({ agent: agent._id })
      .select("amount saleDate")
      .lean();

    // DM transfer history
    const reassignments = await ReassignmentHistory.find({
      agent: agent._id,
    });
    ...
    for (const r of reassignments) {
      if (r.fromManager) {
        dmSalesMap[r.fromManager] =
          (dmSalesMap[r.fromManager] || 0) +
          (r.salesUnderPrevDM || 0);
      }
    }

    if (agent.assignedTo) {
      dmSalesMap[agent.assignedTo] =
        (dmSalesMap[agent.assignedTo] || 0) +
    ...
    return res.status(200).json({
      ...
      sales: {
        lifetime: agent.lifetimeSales || 0,
        dmHistory,
        allSales, // ✅ frontend depends on this
      },
      assignedTo: agent.assignedTo,
    });
  } catch (error: any) {
