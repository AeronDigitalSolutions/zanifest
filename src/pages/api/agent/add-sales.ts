...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // ✅ Prefer reading token from cookies
    ...
    // ✅ Ensure Agent has DM
    if (!agent.assignedTo) {
      return res.status(400).json({ success: false, message: "Agent not assigned to any District Manager" });
    }

    // ✅ Find Manager by managerId (string)
    ...
    return res.status(200).json({
      ...
      sale,
      updatedStats: {
        agentLifetimeSales: agent.lifetimeSales,
        agentSalesUnderDM: agent.currentDMSales,
        managerTotalSales: manager.totalSales,
      },
    });
